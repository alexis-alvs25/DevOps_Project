...
# Retrieve Docker Registry secret from Vault
...
# Retrieve my secrets from Vault
...
export CLOUDS_YAML=$(echo "$SECRET_HOME" | python3 jq.py "data/data/CLOUDS_YAML")
export ANSIBLE_VAULT_PASS=$(echo "$SECRET_HOME" | python3 jq.py "data/data/ANSIBLE_VAULT_PASS")
